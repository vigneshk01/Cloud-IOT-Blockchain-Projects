...
# users_device data import
# Reads users_device.csv one line at a time, splits them into the data fields and inserts
with open(RELATIVE_CONFIG_PATH + USER_DEVICE_COLLECTION + '.csv', 'r') as user_dev_fh:
    for user_dev_row in user_dev_fh:
        user_dev_row = user_dev_row.rstrip()
        if user_dev_row:
            (user_id, device_id, access_type) = user_dev_row.split(',')
            logging.debug(user_id + ' ' + device_id + ' ' + access_type)
            user_dev_coll = weather_dbh[USER_DEVICE_COLLECTION]
            user_dev_coll.update_one({'user_id': user_id}, {
                '$addToSet': {'device_access': {'device_id': device_id, 'access_type': access_type}}},
                                     upsert=True)
