...
class Order:
    ...
    def dispatch(self, delivery_route):
        print(f'Dispatching order {order_request}')
...
class DeliveryMap:
    ...
    def read_config(self, delivery_map_config):
        ...
        for line in dmap_lines:
            ...
            self._destinations.append((destination, delivery_type))
            stages = stages.split(',')
            self._delivery_map[(destination, delivery_type)] = stages
        print(f'Destinations:', (list(dest[0] for dest in self._destinations)))
...
class DeliverySystem:
    ...
    @staticmethod
    def populate_route(center, stages):
        destination = None
        stage_list = []
        for stage in stages:
    ...
    def configure(self, DELIVERY_MAP_CFG):
        delivery_map = DeliveryMap()
        delivery_map.read_config(DELIVERY_MAP_CFG)
...
for order_request in orders:
    dispatch_route = delivery_system.get_route(order_request.city, order_request.delivery_type)
    order_request.dispatch(dispatch_route)
    print('\n')
